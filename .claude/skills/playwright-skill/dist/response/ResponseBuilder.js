...
export class ResponseBuilder {
     ...
     */
    detectAuthStatus(url, cookies, html) {
        // Check for common auth indicators in cookies
        const hasSessionCookie = cookies.some((c) => /session|auth|token|jwt|sid/i.test(c.name));
     ...
     */
    buildPageState(url, title, cookies, html, consoleMessages) {
        return {
            url,
            title,
     ...
     */
    async storeFullContext(command, options) {
        const now = Date.now();
        const fullContext = {
        ...
        const available = ['snapshot', 'console', 'network'];
        if (options.html)
            available.push('html');
    ...
    async build(options) {
        ...
        const pageState = this.buildPageState(url, title, options.cookies ?? [], options.html, options.consoleMessages);
        // Build response
        const response = {
            id: requestId,
     ...
     */
    async success(command, action, options) {
        return this.build({
            ...options,
            command,
            success: true,
     ...
     */
    async error(command, error, options = {}) {
 ...
 */
export function createResponseBuilder(cache, sessionName) {
