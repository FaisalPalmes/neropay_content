 ...
 */
function sessionIsOpen(sessionName) {
    try {
...
function noSessionResponse(sessionName) {
 ...
 */
function runCli(sessionArgs, commandArgs) {
    try {
 ...
 */
function parseFilePath(output, prefix) {
 ...
 */
function mergeConsole(playwrightMsgs, injectedMsgs) {
    if (!injectedMsgs || injectedMsgs.length === 0)
        return playwrightMsgs;
...
async function main() {
    ...
    const networkRequests = parseNetworkLog(networkRaw);
    ...
    // 8. Build and store
    const cookies = normalizeCookies(pageData?.cookies ?? []);
...
function parseInjectedRequests(raw) {
...
function mergeNetwork(playwright, injected) {
