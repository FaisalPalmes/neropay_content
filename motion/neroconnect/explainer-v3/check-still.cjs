...
// ink (pixels darker than the light ground, so the drifting panes behind do not count). Any change fails the build.
  ...
  for (const s of scenes) {
    ...
    for (const k of [0, 1, 2]) {
      ...
      inks.push(await p.evaluate(async b64 => { const img = new Image(); img.src = 'data:image/png;base64,' + b64; await img.decode();
        ...
        const d = x.getImageData(0, 0, c.width, c.height).data; let bits = ''; for (let j = 0; j < d.length; j += 4) bits += (d[j] + d[j + 1] + d[j + 2] < 300) ? '1' : '0';
        return bits; }, buf.toString('base64')));
    }
    const moved = inks[0] !== inks[1] || inks[1] !== inks[2];
