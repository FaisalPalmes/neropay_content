...
(async () => {
  ...
  const film = await mk(`http://127.0.0.1:${port}/motion/partner/ledger/index.html${SQ ? '?fmt=1x1' : ''}`);
  ...
  const { OUTRO_AT, DURATION, arrivals, scenes } = await film.evaluate(() => ({ OUTRO_AT, DURATION,
    arrivals: [...document.querySelectorAll('[data-row]')].map(e => +e.dataset.row).sort((a, b) => a - b),
  ...
  for (const t of arrivals) { if (t - last < 0.45 || t >= OUTRO_AT) continue; last = t;
    cues.push({ sfx: ['click-soft', 'pop'][k++ % 2], t: +(t + 0.05).toFixed(3), gain: 0.06, why: 'a ledger line prints' }); }
  cues.push({ sfx: 'impact-bass-2', t: 28.93 + 0.12, gain: 0.12, why: 'the ACTIVE PARTNER stamp lands' });
  ...
  execFileSync('node', [path.join(REPO, 'motion/mix.mjs'), 'partner/ledger'], { stdio: 'inherit' });
  const name = path.join(OUT, `partner-ledger-${SQ ? '1x1' : PREVIEW ? 'preview' : '16x9'}.mp4`);
