...
const browser = await chromium.launch({ executablePath: resolve(root, build, 'chrome-linux/chrome') });
...
const rows = await pg.evaluate(() => window.lawReport()); const dur = await pg.evaluate(() => window.DURATION);
...
const files = [];
for (const ts of times){ const t = +ts; await pg.evaluate(f => window.setFrame(f), Math.round(t * 30));
