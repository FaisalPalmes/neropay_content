...
| `node motion/tighten.mjs print/mg02 [--gap .34] [--min .5] [--tempo 1.06]` | shrinks every silence in `data/vo.mp3` longer than `--min` to `--gap`, keeps the raw take as `vo-raw.mp3` / `vo_words-raw.json`, rewrites `vo_words.json` and `words.js`; `--tempo` then plays the take faster with the pitch held and scales the timings | run once after the Whisper pass, before any anchor is set |
| `python3 motion/gate.py partner/pp01 [--inset .06] [--max -18]` | the breath gate: mutes what is left between words (inhales, lip noise) with 30 ms ramps, leaving alone any gap whose peak is above `--max` dBFS (a word edge Whisper timed late); keeps the input as `vo-ungated.mp3` | after tighten, before the mix |
| `python3 motion/tempo.py <bed.mp3>` | tempo of a bed by onset autocorrelation (numpy only — librosa isn't in the container or the sandbox); prints bpm, first strong onset, beat length | reads the known 117.45 bpm bed as 120.05 — within the grid's tolerance |
| `node motion/anchors.mjs <episode>/index.html` | dumps `window.ANCHORS` (every word-anchored time), `CAMS`, duration, HEAD and BEAT as JSON | for placing sound cues in `data/mix.json` |
...
| `node motion/render.mjs print/mg02 [--ratio 9x16] [--draft] [--jpeg]` | law check, frame scan, then every frame of every crop → `out/<ep>-<ratio>.mp4` (silent) | ~6 min for four crops on a board; a v5 world is ~12 min a crop with `--jpeg`, run the crops one after another |
...
| `motion/still.mjs` + `motion/lib/preview-terminal.html` | a still of any page (`node motion/still.mjs <page> <out.png> [W H]`), and the three-angle terminal preview used for sign-off | run before an object goes into a render |
| `motion/lib/world3d.js` | the v5 world: `world()` (one scene, perspective camera, CSS3D signs under a transparent WebGL layer, mats and route as DOM planes, the depth-only floor), `section()` (a group with its own box and camera pose, fitted numerically per crop), `flight()` (the wide-view journey between two poses) — `CSS3DRenderer.js` vendored beside three | import from the composition; `print/mg02` is the reference build |
| `motion/lib/objects3d.js` | the 3D objects: `view()`, `neroTerminal()`, `oldTerminal()`, `tiles()`, `pillars()`, `envelope()` (and the retired `terminal()`) — three.js vendored in `motion/assets/vendor/` | import from the composition; `lib/test3d.html` is the smoke test |
