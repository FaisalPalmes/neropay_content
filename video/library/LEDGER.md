...
| `motion/neroconnect/explainer-v3` (NeroConnect explainer, take 3 — v3.5, superseded by take 4) | **Olivia — Warm, British Female** `pPoztmvzd5p26S3MsNrV` | ElevenLabs `eleven_v3`, one continuous take of the whole revised script at Faisal's request (the spliced "minimum" passage sounded different): take 2's text and tags, "minimum" for "floor", and the close "Then check out NeroConnect" added; 153.9 s raw → 130.8 s tightened at 1.03× (the silence guard kept three mistimed words) and gated, 2,272 credits; timed with faster-whisper `small.en` in the Higgsfield sandbox. Raw take `data/vo-take3-raw.mp3`, md5 `15be5ed0d8148dd820a417f2c9e1ca67` | 24 Sep 2026 | same account; flow `V5Z2DUgxnS9rAnIxR1yP`, node `dyHoqZmiifzgRhjoszyR`, generation `w7t0jAYKNipyp2v8N56k` |
| `motion/neroconnect/explainer-v3` (NeroConnect explainer, take 4 — v3.6, the take that ships) | **Olivia — Warm, British Female** `pPoztmvzd5p26S3MsNrV` | ElevenLabs `eleven_v3`, one continuous take of the whole script with Faisal's approved opening ("Look at the card machine on any shop counter. There's a name on it…") and "so no payment is ever priced at a loss" in place of "never lose money"; 167.2 s raw, 2,376 credits. Raw take `data/vo-take4-raw.mp3`, md5 `af483749c21ca40577316f74d1877248`; not yet tightened or timed into the film | 24 Sep 2026 | same account; flow `V5Z2DUgxnS9rAnIxR1yP`, node `puTDibfha8XNXrO2WIvM`, generation `RoQrJLFaHFkVksLfEeqD` |
| `motion/partner/explainer` (Partner Programme explainer for the docs, take 1, superseded by take 2) | **Olivia — Warm, British Female** `pPoztmvzd5p26S3MsNrV` | ElevenLabs `eleven_v3`, one take of the new script written to `motion/partner/BRIEF.md` v4 (every figure with its condition, per merchant, "your bonuses are yours either way"), delivery tags as the NeroConnect v3.6 take; 165.3 s raw → 139.3 s tightened at 1.03× and gated, 2,432 credits; timed with faster-whisper `small.en` in the Higgsfield sandbox. Raw take md5 `7a146627cc8e35c051f9d72d2fcf8e23` | 25 Sep 2026 | same account; flow `V5Z2DUgxnS9rAnIxR1yP`, node `E0E1aiCWVfwvSkJNMQ1O`, generation `DRJiGXu7D52vqJ5G3Txr` |
| `motion/partner/explainer` (Partner Programme explainer, take 2 — v2, the take that ships) | **Olivia — Warm, British Female** `pPoztmvzd5p26S3MsNrV` | ElevenLabs `eleven_v3`, one take of the whole revised script at Faisal's request: no "conditions included", POS for EPOS (written "P.O.S."), the restaurant for the shop (his new photos), wider delivery tags for more range; 161.8 s raw → 136.3 s tightened at 1.03× and gated, 2,474 credits; timed with faster-whisper `small.en` in the Higgsfield sandbox. Raw take `data/vo-take2-raw.mp3`, md5 `7368c1426a71c2c7152b09fc55e7a5bc`; take 1 kept in `data/take1/` | 25 Sep 2026 | same account; flow `V5Z2DUgxnS9rAnIxR1yP`, node `WK7AxxOgtPNJFEUa7iaC`, generation `Sd6f8aF0XPRvJzr26szh` |
| `motion/partner/ledger` (Partner Programme, style B "The Ledger", the take that ships) | **Olivia — Warm, British Female** `pPoztmvzd5p26S3MsNrV` | ElevenLabs `eleven_v3`, one take of the Ledger script; 58.4 s raw → 51.3 s tightened at 1.03× (`--gap .32 --min .45`) and gated (`--inset .12 --max -30`), 922 credits; timed with faster-whisper `small.en` in the Higgsfield sandbox. Raw take `data/vo-raw.mp3`, md5 `52561dfbd0da8859980ffb244dcb760d` | 25 Sep 2026 | same account; flow `V5Z2DUgxnS9rAnIxR1yP`, node `pwSCMkM7pB0auwaK4GVP`, generation `lEAieEdbuLr0XLF1cPLh` |
| `motion/partner/street` (Partner Programme, style C "The High Street", take 1 — v1, superseded by take 2; kept in `data/take1/`) | **Olivia — Warm, British Female** `pPoztmvzd5p26S3MsNrV` | ElevenLabs `eleven_v3`, one take of the High Street script; 60.6 s raw → 51.5 s tightened at 1.03× and gated as above, 919 credits; timed with faster-whisper `small.en` in the Higgsfield sandbox. Raw take `data/vo-raw.mp3`, md5 `640922d935a4484deecfa29e58c662dc` | 25 Sep 2026 | same account; flow `V5Z2DUgxnS9rAnIxR1yP`, node `w4QrSE2lx8ehDYjiqC2b`, generation `QbG7fXsriWVtsHE2V3VT` |
| `motion/partner/street` (Partner Programme, style C "The High Street", take 2 — v2, the take that ships) | **Olivia — Warm, British Female** `pPoztmvzd5p26S3MsNrV` | ElevenLabs `eleven_v3`, one take of the v2 script at Faisal's request (no quiet month, the app and QR steps, the closing question, the bonus said as paid); 73.0 s raw → 62.0 s tightened at 1.03× and gated, 1,033 credits; timed with faster-whisper `small.en` in the Higgsfield sandbox. Raw take `data/vo-raw.mp3`, md5 `4733376eeb0850698651a43f5695e0c3` | 25 Sep 2026 | same account; flow `V5Z2DUgxnS9rAnIxR1yP`, node `ZXwtviCSus8LC1wHfG4N`, generation `fDhjyqDHiNJlMrj2pAAQ` |
