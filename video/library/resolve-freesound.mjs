...
//   FREESOUND_TOKEN=... node resolve-freesound.mjs "soft whoosh" [--out sfx] [--name whoosh-soft] [--any-licence]
// A token comes free from https://freesound.org/apiv2/apply . Without --any-licence only CC0 files are
...
const outDir = path.resolve(opt('--out', 'sfx'));
const anyLicence = args.includes('--any-licence');
const token = process.env.FREESOUND_TOKEN;
if (!query || !token) { console.error('usage: FREESOUND_TOKEN=... node resolve-freesound.mjs "query" [--out dir] [--name file] [--any-licence]'); process.exit(2); }
...
const line = `- \`${path.relative(process.cwd(), file)}\` — "${pick.name}" by ${pick.username}, ${pick.license}, ${pick.url} (query: ${query})`;
fs.appendFileSync(path.join(path.dirname(outDir), 'LEDGER.md'), '\n' + line + '\n');
console.log('\nsaved', file, '\nledger:', line);
