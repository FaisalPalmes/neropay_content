...
/* Tighten every clip: drop the dead air Whisper found between phrases, and the slack before the
   ...
   gets the hold MOTION-SYSTEM.md §3 demands), and (back for v8) the blurred copy next to every cut clip that
   the liquid-glass panels show through themselves. */
...
/* the blurred copy every glass panel shows through itself (build.mjs, bgFor): 480 wide at the clip's own aspect,
   gaussian blur, no audio. The web container's ffmpeg has no blur filter, so there it is a 96-wide copy and the
   page blurs it with CSS (LESSONS.md #27) */
